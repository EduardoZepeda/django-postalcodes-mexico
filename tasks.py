...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-postalcodes-mexico runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-postalcodes-mexico.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_postalcodes_mexico")
...
def lint(c):
    ...
    """
    c.run("flake8 django-postalcodes-mexico tests")
...
def release(c, bumpsize=''):
    ...
    import django_postalcodes_mexico
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_postalcodes_mexico.__version__))
    c.run("git push --tags")
