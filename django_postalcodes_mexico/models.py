...
class PostalCode(TimeStampedModel):
    ...
    D_mnpio = models.CharField(_('Nombre Municipio (INEGI, Marzo 2013)'), max_length=128)
    d_ciudad = models.CharField(_('Nombre Ciudad (Catálogo SEPOMEX)'), max_length=128, blank=True, null=True)
    d_CP = models.CharField(_('Código Postal de la Administración Postal que reparte al asentamiento'), max_length=128, blank=True, null=True)
    c_estado = models.CharField(_('Clave Entidad (INEGI, Marzo 2013)'), max_length=2, choices=C_ESTADO_CHOICES)
    c_oficina = models.CharField(_('Código Postal de la Administración Postal que reparte al asentamiento'), max_length=5)
    c_tipo_asenta = models.CharField(_('Clave Tipo de asentamiento (Catálogo SEPOMEX)'), max_length=2, choices=C_TIPO_ASENTA_CHOICES)
    c_mnpio = models.CharField(_('Clave Municipio (INEGI, Marzo 2013)'), max_length=3)
    id_asenta_cpcons = models.CharField(_('Identificador único del asentamiento (nivel municipal)'), max_length=4)
    d_zona = models.CharField(_('Zona en la que se ubica el asentamiento (Urbano/Rural)'), max_length=10, choices=D_ZONA_CHOICES)
    c_cve_ciudad = models.CharField(_('Clave Ciudad (Catálogo SEPOMEX)'), max_length=2, blank=True, null=True)
